...
General Compute 转 OpenAI 兼容 API，内置账号池与管理后台。

## 致谢与声明

本项目在 **[ds2api](https://github.com/CJackHwang/ds2api)**（作者 [CJackHwang](https://github.com/CJackHwang)）基础上二次开发并重命名为 GeneralCompute2API
...
# 编辑 .env：设置 GENERALCOMPUTE2API_ADMIN_TOKEN
...
```bash
go run ./cmd/generalcompute2api   # :8000
go test ./...
go run ./cmd/generalcompute2api-tests # 集成测试
```
...
| http://127.0.0.1:5103/admin/ | 号池 WebUI |

支持 OpenAI Chat Completions / Responses 风格接口。高强度使用每天禁言2-3个号，建议50个号起用（批量注册参考我的仓库 `signup-god`）
