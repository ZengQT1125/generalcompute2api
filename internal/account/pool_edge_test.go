...
func TestPoolEmptyNoAccounts(t *testing.T) {
	t.Setenv("GENERALCOMPUTE2API_ACCOUNT_MAX_INFLIGHT", "2")
	t.Setenv("GENERALCOMPUTE2API_ACCOUNT_MAX_QUEUE", "")
	pool := newTestPool(nil, config.LoadStore())
...
func TestPoolMaxQueueSizeOverride(t *testing.T) {
	...
	t.Setenv("GENERALCOMPUTE2API_ACCOUNT_MAX_QUEUE", "5")
	pool := newTestPool([]config.Account{{Email: "acc1@example.com", Token: "t1"}}, config.LoadStore())
	status := pool.Status()
