...
import (
	...
	trans "generalcompute2api/internal/deepseek/transport"
)
...
func (c *Client) UploadFile(ctx context.Context, a *auth.RequestAuth, req UploadFileRequest, maxAttempts int) (*UploadFileResult, error) {
	releaseUpload, err := globalUploadLimiter.acquire(ctx)
	if err != nil {
		return nil, err
	}
	defer releaseUpload()
	...
	modelType := config.UpstreamSafeModelType(req.ModelType)
	...
	for attempts < maxAttempts {
		...
		attempts++
		if shouldBackoffUploadFailure(resp.StatusCode, msg, bizMsg) && attempts < maxAttempts {
			sleepUploadBusyBackoff(ctx)
		}
	}
...
var uploadBusyBackoffSleep = func(ctx context.Context, d time.Duration) {
...
func shouldBackoffUploadFailure(status int, msg string, bizMsg string) bool {
	if status == http.StatusTooManyRequests || status == http.StatusServiceUnavailable {
		return true
	}
	combined := strings.ToLower(strings.TrimSpace(msg) + " " + strings.TrimSpace(bizMsg))
	return strings.Contains(combined, "server is busy") ||
		strings.Contains(combined, "busy") ||
		strings.Contains(combined, "rate limit") ||
		strings.Contains(combined, "too many requests")
}

func sleepUploadBusyBackoff(ctx context.Context) {
	d := 800*time.Millisecond + time.Duration(rand.Intn(401)-200)*time.Millisecond
	if d < 0 {
		d = 0
	}
	uploadBusyBackoffSleep(ctx, d)
}
