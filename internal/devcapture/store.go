...
import (
	...
	"generalcompute2api/internal/util"
...
func NewFromEnv() *Store {
	enabled := true
	if raw, ok := os.LookupEnv("GENERALCOMPUTE2API_DEV_PACKET_CAPTURE"); ok {
		enabled = parseBool(raw)
	}
	limit := parseIntWithDefault(os.Getenv("GENERALCOMPUTE2API_DEV_PACKET_CAPTURE_LIMIT"), defaultLimit)
	if limit < 1 {
		limit = defaultLimit
	}
	if limit > maxLimit {
	...
	maxBodyBytes := parseIntWithDefault(os.Getenv("GENERALCOMPUTE2API_DEV_PACKET_CAPTURE_MAX_BODY_BYTES"), defaultMaxBodyBytes)
