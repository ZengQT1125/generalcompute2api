...
import (
	...
	"generalcompute2api/internal/assistantturn"
	openaifmt "generalcompute2api/internal/format/openai"
	"generalcompute2api/internal/httpapi/openai/shared"
	"generalcompute2api/internal/promptcompat"
	"generalcompute2api/internal/sse"
	streamengine "generalcompute2api/internal/stream"
	"generalcompute2api/internal/toolstream"
)
...
func (s *chatStreamRuntime) attachUpstreamEmptyDiagnostics(payload map[string]any, clientContentLength *int64) {
	if s == nil {
		return
	}
	s.diagCompletionPayload = payload
	s.diagClientContentLength = clientContentLength
}
...
func (s *chatStreamRuntime) finalize(finishReason string, deferEmptyOutput bool) bool {
	...
	if outcome.ShouldFail {
		...
		if outcome.Error != nil && outcome.Error.Code == "upstream_empty_output" {
			surf := strings.TrimSpace(s.logSurface)
			if surf == "" {
				surf = "openai_chat"
			}
			assistantturn.LogUpstreamEmptyOutputDiagnostic(surf, true, "chat_stream_finalize", turn, s.diagCompletionPayload, s.diagClientContentLength)
		}
