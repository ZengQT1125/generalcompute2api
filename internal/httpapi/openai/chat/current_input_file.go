...
import (
	...
	"generalcompute2api/internal/privatecontext"
...
func (h *Handler) applyCurrentInputFile(ctx context.Context, a *auth.RequestAuth, stdReq promptcompat.StandardRequest) (promptcompat.StandardRequest, error) {
	...
	if err != nil {
		config.Logger.Warn("[current_input_file] private context upload failed; falling back to inline prompt", "surface", "openai.chat", "error", err)
		return stdReq, nil
	}
	fileID := strings.TrimSpace(result.ID)
	if fileID == "" {
		config.Logger.Warn("[current_input_file] private context upload returned empty file id; falling back to inline prompt", "surface", "openai.chat")
		return stdReq, nil
	}
