...
// newTwoCallerAPIKeyResolver registers two GeneralCompute2API keys (no overlap with managed-key tests).
func newTwoCallerAPIKeyResolver(t *testing.T) (*config.Store, *auth.Resolver) {
	t.Helper()
	store := config.LoadStore()
	accounts := []config.Account{{Email: "acc@example.com", Password: "pwd", Token: "account-token"}}
	mem := pooldb.NewMem()
	mem.RegisterKey("token-a", accounts, true)
	mem.RegisterKey("token-b", accounts, true)
...
func newManagedKeyResolver(t *testing.T) (*config.Store, *auth.Resolver) {
	...
	store := config.LoadStore()
	mem := pooldb.NewMem()
	mem.RegisterKey("managed-key", []config.Account{{
		Email: "acc@example.com", Password: "pwd", Token: "account-token",
	}}, true)
	resolver := auth.NewResolver(store, func(_ context.Context, _ config.Account) (string, error) {
		return "unused", nil
	})
...
func authCallerForToken(t *testing.T, resolver *auth.Resolver, token string) *auth.RequestAuth {
	t.Helper()
	req := httptest.NewRequest(http.MethodGet, "/v1/responses/resp_test", nil)
	req.Header.Set("Authorization", "Bearer "+token)
	a, err := resolver.DetermineCaller(req)
	if err != nil {
...
func TestGetResponseByIDRequiresAuthAndIsTenantIsolated(t *testing.T) {
	store, resolver := newTwoCallerAPIKeyResolver(t)
	h := &Handler{Store: store, Auth: resolver}
	r := chi.NewRouter()
	RegisterRoutes(r, h)

	ownerA := responseStoreOwner(authCallerForToken(t, resolver, "token-a"))
