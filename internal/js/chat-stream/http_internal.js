...
function internalSecret() {
  return asString(process.env.GENERALCOMPUTE2API_VERCEL_INTERNAL_SECRET) || asString(process.env.GENERALCOMPUTE2API_ADMIN_KEY) || 'admin';
}
...
function buildInternalGoHeaders(req, opts = {}) {
  const headers = {
    ...
    'x-generalcompute2api-target-account': asString(header(req, 'x-generalcompute2api-target-account')),
    'x-vercel-protection-bypass': resolveProtectionBypass(req),
  };
  if (opts.withInternalToken) {
    headers['x-generalcompute2api-internal-token'] = internalSecret();
  }
...
function resolveProtectionBypass(req) {
  ...
  return asString(process.env.VERCEL_AUTOMATION_BYPASS_SECRET) || asString(process.env.GENERALCOMPUTE2API_VERCEL_PROTECTION_BYPASS);
}
