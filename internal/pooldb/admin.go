...
func (db *DB) AddAccountToPool(ctx context.Context, apiKey, identifier, password string) error {
	...
	var accPassword, accCookie, accSessionID, accOrgID string
	accPassword = password

	// 完美解析 GL 高级 JSON 导入格式
	if strings.HasPrefix(strings.TrimSpace(password), "{") {
		...
		if err := json.Unmarshal([]byte(password), &extra); err == nil {
			accPassword = extra.Password
			accCookie = extra.Cookie
			accSessionID = extra.SessionID
			accOrgID = extra.OrganizationID
		}
	}

	var accountID int64
	err = tx.QueryRowContext(ctx, `
INSERT INTO pool_accounts (identifier, password, cookie, session_id, organization_id) VALUES (?, ?, ?, ?, ?)
ON CONFLICT (identifier) DO UPDATE SET
  password = CASE WHEN excluded.password <> '' THEN excluded.password ELSE pool_accounts.password END,
  cookie = CASE WHEN excluded.cookie <> '' THEN excluded.cookie ELSE pool_accounts.cookie END,
  session_id = CASE WHEN excluded.session_id <> '' THEN excluded.session_id ELSE pool_accounts.session_id END,
  organization_id = CASE WHEN excluded.organization_id <> '' THEN excluded.organization_id ELSE pool_accounts.organization_id END,
  discarded = 0
RETURNING id
`, identifier, accPassword, accCookie, accSessionID, accOrgID).Scan(&accountID)
	if err != nil {
		return err
	}
