...
// DefaultDatabasePath is used when GENERALCOMPUTE2API_DATABASE_PATH is unset (local dev).
const DefaultDatabasePath = "docker-data/generalcompute2api/generalcompute2api.db"
...
func databasePathFromEnv() string {
	if p := strings.TrimSpace(os.Getenv("GENERALCOMPUTE2API_DATABASE_PATH")); p != "" {
		return p
	}
...
func sqliteDSN(path string) string {
	// WAL + busy_timeout help generalcompute2api and poolui share one file.
...
func maxAccountsPerKey() int {
	raw := strings.TrimSpace(os.Getenv("GENERALCOMPUTE2API_POOL_MAX_ACCOUNTS_PER_KEY"))
	if raw == "" || raw == "0" {
		return 0
	}
