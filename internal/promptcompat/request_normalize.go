...
func NormalizeOpenAIChatRequest(req map[string]any, traceID string) (StandardRequest, error) {
	...
	resolvedModel, ok := config.ResolveModel(model)
	if !ok {
...
func NormalizeOpenAIResponsesRequest(req map[string]any, traceID string) (StandardRequest, error) {
	model, _ := req["model"].(string)
