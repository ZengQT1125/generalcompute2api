...
func TestCORSPreflightAllowsThirdPartyRequestedHeaders(t *testing.T) {
	...
	req.Header.Set("Access-Control-Request-Headers", "authorization, x-stainless-os, x-stainless-runtime, x-generalcompute2api-internal-token")
	...
	if strings.Contains(allowHeaders, "x-generalcompute2api-internal-token") {
...
func TestBuildCORSAllowHeadersKeepsDefaultsWithoutRequest(t *testing.T) {
	...
	for _, want := range []string{"content-type", "authorization", "x-api-key", "x-generalcompute2api-source"} {
...
func TestAppCORSPreflightIsUnifiedAcrossInterfaces(t *testing.T) {
	t.Setenv("GENERALCOMPUTE2API_ENV_WRITEBACK", "0")
