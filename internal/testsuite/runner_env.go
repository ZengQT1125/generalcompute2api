...
func (r *Runner) seedGatewayPool(ctx context.Context) error {
	path := strings.TrimSpace(os.Getenv("GENERALCOMPUTE2API_DATABASE_PATH"))
	if path == "" {
...
func (r *Runner) startServer(ctx context.Context) error {
	...
	r.serverLogFd = logFd
	cmd := exec.CommandContext(ctx, "go", "run", "./cmd/generalcompute2api")
	cmd.Stdout = logFd
	cmd.Stderr = logFd
	dbPath := strings.TrimSpace(os.Getenv("GENERALCOMPUTE2API_DATABASE_PATH"))
	if dbPath == "" {
		dbPath = pooldb.DefaultDatabasePath
	}
	cmd.Env = prepareServerEnv(os.Environ(), map[string]string{
		"PORT":                                strconv.Itoa(port),
		"GENERALCOMPUTE2API_AUTO_BUILD_WEBUI": "false",
		"GENERALCOMPUTE2API_DATABASE_PATH":    dbPath,
	})
