...
const handler = require('./chat-stream.js');
...
test('vercel stream does not same-account synthetic-retry empty output', async () => {
  ...
  assert.equal(fetchURLs.filter((url) => url.includes('__stream_pow=1')).length, 0);
...
test('vercel stream reuses prior PoW when continue refresh fails', async () => {
  const originalFetch = global.fetch;
  const fetchURLs = [];
  const deepseekPowHeaders = [];
  let completionCalls = 0;
  global.fetch = async (url, init = {}) => {
    const textURL = String(url);
    fetchURLs.push(textURL);
    ...
    if (textURL === 'https://chat.deepseek.com/api/v0/chat/completion') {
      deepseekPowHeaders.push(init.headers['x-ds-pow-response']);
      completionCalls += 1;
      ...
      throw new Error(`unexpected completion call: ${completionCalls}`);
    }
    if (textURL === 'https://chat.deepseek.com/api/v0/chat/continue') {
      deepseekPowHeaders.push(init.headers['x-ds-pow-response']);
      return sseResponse(['data: {"p":"response/content","v":"continued"}\n\n', 'data: [DONE]\n\n']);
    }
  ...
  try {
    ...
    const parsed = frames.filter((frame) => frame !== '[DONE]').map((frame) => JSON.parse(frame));
    assert.deepEqual(deepseekPowHeaders, ['pow-header-initial', 'pow-header-initial']);
    assert.equal(fetchURLs.filter((url) => url.includes('__stream_pow=1')).length, 1);
    assert.equal(parsed[0].choices[0].delta.content, 'continued');
    assert.equal(parsed[1].choices[0].finish_reason, 'stop');
  } finally {
...
test('setCorsHeaders reflects requested third-party headers and blocks internal-only headers', () => {
  ...
  setCorsHeaders(res, {
    headers: {
      ...
      'access-control-request-headers': 'authorization, x-stainless-os, x-stainless-runtime, x-generalcompute2api-internal-token',
  ...
  assert.equal(allowHeaders.includes('x-generalcompute2api-internal-token'), false);
