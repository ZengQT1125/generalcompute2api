...
POOL_FIXTURE="${1:-tests/fixtures/gateway-pool.json}"
SAMPLE_ID="${2:-capture-$(date -u +%Y%m%dT%H%M%SZ)}"
QUESTION="${3:-广州天气}"
MODEL="${4:-deepseek-v4-pro-search}"
API_KEY="${5:-}"
ADMIN_KEY="${GENERALCOMPUTE2API_ADMIN_KEY:-admin}"

if [[ -z "$API_KEY" ]]; then
  API_KEY="$(python3 - <<'PY' "$POOL_FIXTURE"
import json,sys
...
cleanup() {
  ...
  pkill -f "cmd/generalcompute2api" >/dev/null 2>&1 || true
}
trap cleanup EXIT

export GENERALCOMPUTE2API_DATABASE_PATH="${GENERALCOMPUTE2API_DATABASE_PATH:-docker-data/generalcompute2api/generalcompute2api.db}"

GENERALCOMPUTE2API_DEV_PACKET_CAPTURE=1 \
GENERALCOMPUTE2API_DEV_PACKET_CAPTURE_LIMIT=20 \
  go run ./cmd/generalcompute2api >/tmp/generalcompute2api_capture_server.log 2>&1 &
...
for line in headers:
  if line.lower().startswith('x-generalcompute2api-sample-dir:'):
...
headers=pathlib.Path(sys.argv[1]).read_text().splitlines()
for line in headers:
  if line.lower().startswith('x-generalcompute2api-sample-id:'):
    print(line.split(':',1)[1].strip())
    raise SystemExit
print('')
PY
